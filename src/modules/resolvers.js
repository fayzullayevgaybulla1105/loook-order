...
export const resolvers = {
    ...
    User: {
        ...
        orders: global => {
            let orders = read('orders')
            // orders.json ning ichidagi har bir orderning user_id sini globalga kelayotgan user_idd bn tengligini filter orqali tekshiramiz;
            return orders.filter(order => global.user_id == order.user_id)
    ...
    Order: {
        ...
        food: global => read('foods').find(food => global.food_id == food.food_id),
        user: global => read('users').find(user => global.user_id == user.user_id)
